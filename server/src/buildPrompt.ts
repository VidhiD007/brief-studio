...
Ground every field in what is actually visible or actually stated — never invent room names, directions, or issues that aren't supported by the image or the manual fields. If the floor plan is genuinely illegible, say that honestly in the relevant fields rather than fabricating specifics. Keep each field to 1-2 sentences — for a large or multi-unit floor plan with many rooms, summarize by zone or room type (e.g. "3 bedrooms, 2 bathrooms, open kitchen-living area" per unit) rather than naming every single room individually.`;
